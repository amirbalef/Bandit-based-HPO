...
class BanditOptimizer():
    def __init__(self, space, subsapce_optimizer, subsapce_optimizer_parameters ,policy, policy_parameters ,seed, initial_configs = None, limit_to_configs = None,):
        self.space = space
        ...
        self.subsapces, self.arms_names, self.parent_names  = make_subspaces_by_conditions(space)
        ...
        for i, sub_space in enumerate(self.subsapces):
            if('n_configs' in self.subsapce_optimizer_parameters):
                if(self.subsapce_optimizer_parameters['n_configs']==None):
                    self.subsapce_optimizer_parameters['n_configs'] = self.subsapce_optimizer_parameters["n_trials"]//(self.number_of_arms*4)

            if(self.initial_configs!=None):
                config =  dict(self.initial_configs[i])
                del config[self.parent_names[i]]
                initial_configs  = [ConfigSpace.configuration_space.Configuration(sub_space, values=config)]
                ...
                for config in self.limit_to_configs:
                    if(self.parent_names[i] in config):
                        if(config[self.parent_names[i]] == self.arms_names[i]):
                            ...
                            new_config = ConfigSpace.configuration_space.Configuration(sub_space, values=new_config)
                            limit_to_configs.append(new_config)
            self.subspace_optimizers.append(self.subsapce_optimizer(space=sub_space, **self.subsapce_optimizer_parameters, initial_configs = initial_configs, limit_to_configs = limit_to_configs))
    ...
    def ask(self):
        ...
        trial.name = 'exp'+ str(self.t) +'-'+ self.arms_names[self.selected_arm ] +'-'+ trial.name
        trial.config[self.parent_names[self.selected_arm]]= self.arms_names[self.selected_arm ]
        return trial
